...
public class QueryHelper<T extends AbstractBaseEntity> {
    ...
    public QueryHelper(BaseService service, RedisHelper redisHelper) {
        this.service = service;
        this.redisHelper = redisHelper;
    }

    private AbstractBaseEntity selectOne(T entity) {
        AbstractBaseEntity res;
        if (selectOneCallBack != null)
            res = selectOneCallBack.selectOne(entity);
        else
            res = service.selectOne(entity);
        return res == null ? null : res.setFromDB(true);
    }
    ...
    public Result<T> readDBEntity(T entity) {
        AbstractBaseEntity resEntity = selectOne(entity);
        if (resEntity == null)
            return new Result(FAIL).setErrorCode(1010110101).setMessage(entity.getClass().getSimpleName() + "对象" + entity.getId() + "不存在");
        return new Result(SUCCESS).setObject(resEntity);
    }

    public Result<T> readEntity(@NotNull T entity, boolean isFull) {
        AbstractBaseEntity resEntity = redisHelper.getEntity(entity.getId(), isFull, false);
        if (resEntity == null) {
            if (isFull)
    ...
    public Result<List<T>> multiReadEntity(@NotNull List<Integer> idList, boolean isFull, T queryEntity) {
        ...
        if (dbEntityList == null)
            return new Result(Result.FAIL).setErrorCode(1010110103).setMessage("数据库操作失败");
