...
public class QueryTokenUtil {
    ...
    public static Result validateTokenAndSetQueryParam(@NotNull AbstractBaseEntity param, int indexType, String hmacKey) {
        ...
        if (token != null) success:{
            ...
            if (splitStr.length != 2)
                return new Result(Result.FAIL).setErrorCode(1010140201).setMessage("Token格式不正确");//格式不正确，直接返回错误
            if (!splitStr[1].equals(CryptUtil.hmac(hmacKey, splitStr[0])))
                return new Result(Result.FAIL).setErrorCode(1010140202).setMessage("Token校验失败");//HMAC签名验证失败，直接返回错误
            ...
            if ("1".equals(splitParam[0]))//产生token是设置的abandon位，说明刷到底了
                return new Result(Result.FAIL).setErrorCode(1010140203).setMessage("没有更多内容");
            if (new Date().getTime() - Long.valueOf(splitParam[1]) < 500)
                return new Result(Result.FAIL).setErrorCode(1010140204).setMessage("刷新过于频繁");;//两次更新时间小于500毫秒，直接返回错误
            if (!param.getEntityType().equals(splitParam[2]))
                return new Result(Result.FAIL).setErrorCode(1010140205).setMessage("queryToken类型不符");
