...
public class QueryTokenUtil {
    ...
    public static <T extends AbstractBaseEntity> String generateQueryToken(@NotNull T param, @NotNull List<T> resList, @NotNull QueryParam queryParam, int indexType, String hmacKey) {
        ...
        // 刷到底了，该token弃用
        boolean abandon = endOfQuery || resList.size() == 0;
        ...
        String ori = (abandon ? '1' : '0') + "#" + System.currentTimeMillis() + "#" + param.getEntityType() + "#" + indexType
        ...
        if (token != null) {
            success:{
                ...
                if (splitStr.length != 2) {
                    // 格式不正确，直接返回错误
                    return new Result<Object>(Result.FAIL).setErrorCode(1010140201).setMessage("Token格式不正确");
                }
                if (!splitStr[1].equals(CryptUtil.hmac(hmacKey, splitStr[0]))) {
                    // HMAC签名验证失败，直接返回错误
                    return new Result<Object>(Result.FAIL).setErrorCode(1010140202).setMessage("Token校验失败");
                }
                String[] splitParam = splitStr[0].split("#");
                if (splitParam.length != 9) {
                    // 参数格式不正确，可能是后端的问题，设为默认值
                    break success;
                }
                // 产生token是设置的abandon位，说明刷到底了
                if ("1".equals(splitParam[0])) {
                    return new Result<Object>(Result.FAIL).setErrorCode(1010140203).setMessage("没有更多内容");
                }
                if (System.currentTimeMillis() - Long.valueOf(splitParam[1]) < 500) {
                    return new Result<Object>(Result.FAIL).setErrorCode(1010140204).setMessage("刷新过于频繁");
                }
                ;//两次更新时间小于500毫秒，直接返回错误
